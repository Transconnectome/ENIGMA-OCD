...
# in train set
shapiro.test(train_pred$Dx.1) # W = 0.98034, p-value = 0.001423 (*high p-value means normality OK)
qqnorm(train_pred$Dx.1)
qqline(train_pred$Dx.1)

shapiro.test(train_pred_HC$Dx.1) # W = 0.97195, p-value = 0.01767
qqnorm(train_pred_HC$Dx.1)
qqline(train_pred_HC$Dx.1)

shapiro.test(train_pred_OCD$Dx.1) # W = 0.97888, p-value = 0.02885
qqnorm(train_pred_OCD$Dx.1)
qqline(train_pred_OCD$Dx.1)

# in test set
shapiro.test(test_pred$Dx.1) # W = 0.96363, p-value = 0.05626
qqnorm(test_pred$Dx.1)
qqline(train_pred$Dx.1)

shapiro.test(test_pred_HC$Dx.1) # W = 0.975, p-value = 0.7006
qqnorm(test_pred_HC$Dx.1)
qqline(test_pred_HC$Dx.1)

shapiro.test(test_pred_OCD$Dx.1) # W = 0.9431, p-value = 0.06957
...
train_pred <- train_pred %>%
  mutate(threshold = ifelse(Site == 'Bangalore', 0.4313105,
                            ifelse(Site == 'Barcelona', 0.559111,
                                   ifelse(Site == 'British Columbia', 0.3878554,
                                          ifelse(Site == 'Calgary', 0.5781595,
                                                 ifelse(Site == 'Chiba', 0.4980515,
                                                        ifelse(Site == 'Oxford', 0.5619532,
                                                               ifelse(Site == 'Yale', 0.4215366,
                                                                      ifelse(Site == 'Zurich', 0.5133553, NA)))))))))
