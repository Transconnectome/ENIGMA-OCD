...
test_harmonized <- neuroCombat(dat = test_brain_z_t, batch = test_batch, parametric = FALSE, mean.only = TRUE)
...
test_harmonized.cov_as <- neuroCombat(dat=test_brain_z_t, batch = test_batch, mod = test_mod, mean.only = TRUE)
...
# test set
write.csv(test_harmo, file="Pedi_UnmeOCDHC_test_harmo.csv")
write.csv(test_harmo.cov_as, file="Pedi_UnmedOCDHC_test_harmo_cov.csv")
