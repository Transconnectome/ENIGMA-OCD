...
library(magrittr)
library(pROC)
setwd("C:/Users/김보겸/Desktop/Connectome/study-enigma ocd/ENIGMA-OCD/0.Data/Dai result/After piras updated_Pediatric/MedHC_pedi_LOSO")


#### 2. Load data 
proba_val <- read_csv('h2oai_experiment_MedHC_pedi_LOSO_train_predictions.csv')
proba_test <- read_csv('h2oai_experiment_MedHC_pedi_LOSO_test_predictions.csv')

#### 3. Validation result performance ####
#### site별 데이터가 있는지 파악 => 없는 데이터: Amsterdam, Kyoto, Shanghai    
...
sites <- c("Bangalore"  ,      "Barcelona"  ,      "British Columbia",          "Chiba","Yale",             "Zurich"         )
...
for (i in c(1:n_site)) {
  site_roc <- proba_val %>% filter(Site == sites[i]) %>% roc(MedHC, MedHC.1) 
